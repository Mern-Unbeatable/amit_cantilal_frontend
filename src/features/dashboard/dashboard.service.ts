import type {
  ...
  MonthlyBooking,
  ServiceStatistic,
} from '@/features/dashboard/dashboard.types.ts'
...
export const dashboardService = {
  ...
  getMonthly: async () => {
    const response = await api.get<{ data: Array<MonthlyBooking> }>(
  ...
  getChannels: async () => {
    const response = await api.get<{ data: Array<ServiceStatistic> }>(
  ...
  getRecentBookings: async () => {
    const response = await api.get<{ data: Array<BookingState> }>(
