...
function Co2Calculator() {
  ...
  const distanceOptions = (
    t('sustainabilityImpact.calculator.distanceOptions', {
      returnObjects: true,
  ...
  const results = [
    {
      icon: Car,
      label: t('sustainabilityImpact.calculator.monthlyLabel'),
      value: `${monthlySaving} kg`,
      unit: 'CO₂',
    },
    {
      icon: TrendingDown,
      label: t('sustainabilityImpact.calculator.yearlyLabel'),
      value: `${yearlySaving} kg`,
      unit: 'CO₂',
    },
    {
      icon: Leaf,
      label: t('sustainabilityImpact.calculator.treesLabel'),
      value: treesEquiv,
      unit: t('sustainabilityImpact.calculator.treesUnit'),
    },
  ]
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-[#0B0B0B] border-2 border-green-500/25 p-6 md:p-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Calculator
            className="w-6 h-6 md:w-8 md:h-8 text-[#C9A84C]"
            strokeWidth={1.5}
          />
        ...
        <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
          ...
          <div className="flex flex-col gap-2">
            <Label
              htmlFor="trips"
              className="text-xs md:text-sm font-medium tracking-[.12em] uppercase text-[#9A9182]"
            >
        ...
        <div className="grid md:grid-cols-3 gap-4 md:gap-6">
          {results.map(({ icon: Icon, label, value, unit }) => (
            ...
            >
              <Icon
                className="w-8 h-8 md:w-10 md:h-10 text-green-500 mb-3 mx-auto"
                strokeWidth={1.5}
              />
              <p className="text-xs md:text-sm text-[#9A9182] mb-2">{label}</p>
              <p className="font-serif text-2xl md:text-3xl font-light text-gradient-gold">
                {value}
              </p>
              <p className="text-[10px] md:text-xs text-[#9A9182] mt-1">
                {unit}
              </p>
            </div>
          ))}
...
export default function SustainabilityImpact() {
  const { t } = useTranslation()
  const highlights = (
    t('sustainabilityImpact.highlights', { returnObjects: true }) as Array<{
      title: string
      description: string
    }>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-6 md:mb-16">
            ...
            <h2 className="font-serif text-2xl md:text-5xl lg:text-6xl font-light text-gradient-gold mb-3 md:mb-6">
              {t('sustainabilityImpact.titlePrefix')}{' '}
              <em className="italic">
                {t('sustainabilityImpact.titleEmphasis')}
              </em>
            </h2>
            ...
            {highlights.map(({ icon: Icon, title, description }) => (
              ...
              >
                <div className="inline-flex items-center justify-center w-10 h-10 md:w-20 md:h-20 bg-[#C9A84C] mb-2 md:mb-4">
                  <Icon
                    className="w-5 h-5 md:w-9 md:h-9 text-[#0B0B0B]"
                    strokeWidth={1.5}
                  />
                </div>
