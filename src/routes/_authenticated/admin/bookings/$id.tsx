  ...
  return (
    <Card className="p-7 gap-0">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-9 h-9 rounded-full bg-primary/10 border border-primary/25 text-primary flex items-center justify-center shrink-0">
          {icon}
        </div>
...
function RouteComponent() {
  ...
  const handleResendConfirmation = () => {
    resendConfirmation(booking.id, {
      onSuccess: () =>
        toast.success(
          'Confirmation resent to the customer (email + WhatsApp).',
        ),
              ...
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                  ...
                  {(details?.vehicle_name || details?.vehicle_type) && (
                    <Field
                      label="Vehicle"
                      value={details.vehicle_name ?? details.vehicle_type}
                      icon={
                  ...
                  {booking.status === 'cancelled' &&
                    booking.payment.status === 'succeeded' && (
                      <div className="flex items-start gap-2 mb-4 p-3 rounded-md border border-destructive/30 bg-destructive/10 text-destructive text-sm">
                        ...
                        <span>
                          This booking was cancelled but the payment hasn't been
                          refunded yet — issue the refund manually from the
                          Stripe dashboard.
                        </span>
                      </div>
                    )}
              ...
              >
                <div className="bg-[#F7F1E6] rounded-sm px-5 py-1">
                  <div className="flex items-center justify-between py-3 border-b border-black/10">
                    <span className="text-[11px] uppercase tracking-wider text-[#6b5f45]">
                      Reference
                    </span>
